...
{    
  ...
  // DEBUG <<<
     ...
     // die();
  // >>> DEBUG                  
    // Mage::app()->getFrontController()->getResponse()->setRedirect($_SERVER['REQUEST_URI'] . '&fromAddToCart=1');    
  }
  // DEBUG <<<
     // echo '$_REQUEST'; echo '<hr>'; print_r($_REQUEST); echo '<hr>';    
  // >>> DEBUG  
...
public function getCategoriesTree()
{
  ...
  if ($ids)  
    ...
    if ($id > 2)    
    {
      ...
      $objCategory->load($id);      
      if (
        $objCategory->getIncludeInMenu() 
        && $objCategory->getIsActive()
        && ($objCategory->getLevel() < 4)
      )
      {
        // DEBUG <<<      
