...
{
    if (showforgoterreur) {
        $("divajaxforgoterreur").hide();
        showforgoterreur = false;
    }
    else {
        $("divajaxforgoterreur").show();
...
{
    if (showcreateerreur) {
        $("divajaxcreateerreur").hide();
        showcreateerreur = false;
    }
    else {
        $("divajaxcreateerreur").show();
...
function ajaxlogin()
{
    ...
    formkey__ = $("ajaxformkey").value;

    if (testemail(email__))
    {
        message_erreur = 'Неправильная эл. почта.<br>';
        test1 = true;
    ...
    new Ajax.Request(url_ajax, {
        method: 'post',
        parameters: {'login[password]': pass__, 'login[username]': email__, 'form_key': formkey__},
...
function ajaxcreate()
{
    password__ = $("ajaxpassword").value;
    password_empty = false;
    confirmation__ = $("ajaxconfirmation").value;
    confirmation_empty = false;
    is_subscribed__ = $("ajaxis_subscribed").value;
    email__ = $("ajaxemail_address").value;
    email_empty = false;
    lastname__ = $("ajaxlastname").value;
    lastname__empty = false;
    firstname__ = $("ajaxfirstname").value;
    firstname_empty = false;

    if (password__ == '')
        password_empty = true;
    if (confirmation__ == '')
        confirmation_empty = true;
    if (email__ == '')
        email_empty = true;
    if (lastname__ == '')
        lastname__empty = true;
    if (firstname__ == '')
            ...
            firstname_empty)
    {
        ...
        if (password_empty)
            $("ajaxpassword").writeAttribute('style', 'border:1px solid red;');
        if (confirmation_empty)
            $("ajaxconfirmation").writeAttribute('style', 'border:1px solid red;');
        if (email_empty)
            $("ajaxemail_address").writeAttribute('style', 'border:1px solid red;');
        if (lastname__empty)
            $("ajaxlastname").writeAttribute('style', 'border:1px solid red;');
        if (firstname_empty)
            $("ajaxfirstname").writeAttribute('style', 'border:1px solid red;');
    ...
    {
        message_erreur = message_erreur + 'Проверьте Ваш пароль <br>';
        test1 = true;
    ...
    if (testemail(email__))
    {
        message_erreur = message_erreur + 'Неправильная эл. почта..<br>';
        test1 = true;
        /* Invalid email address */
        showlogin = true;
        showcreate = false;
    ...
    {
        message_erreur = message_erreur + 'Пароль должен быть не менее 6 символов<br>';
        test2 = true;
...
function ajaxforgot()
{
    email__ = $("ajaxemail_address_forgot").value;
    email_empty = false;
    ...
    if (email_empty)
    {
        ...
        if (email_empty)
            $("ajaxemail_address_forgot").writeAttribute('style', 'border:1px solid red;');
